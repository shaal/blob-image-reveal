 ...
 * 2. Tracking mouse position and moving the blob mask
...
function applyBlobSize() {
  ...
  if (desiredPixels && blobScale) {
    // Get current container width
    const containerWidth = container.getBoundingClientRect().width;

    // Scale factor: compensate for SVG scaling to maintain fixed pixel size
    // When container is smaller than viewBox, we need to scale UP the blob
...
// Recalculate blob size when window resizes
window.addEventListener('resize', applyBlobSize);
...
// Get smoothing factor from CSS variable
function getSmoothing() {
  const value = getComputedStyle(document.documentElement).getPropertyValue('--blob-smoothing').trim();
  return parseFloat(value) || 0.08;
}
